...
Stock Graphify v8 (0.9.61 as of 2026-09-12) still skips `.ipynb` — it is in
neither `CODE_EXTENSIONS` nor `DOC_EXTENSIONS`. 0.9.61 is a fix-only release
for `graphify.serve` on Python 3.12/3.13 (`jieba-py` extra) and the git hook's
rebuild-root guard against a symlink-loop or dangling `.graphify_root`. Native
support is proposed in
